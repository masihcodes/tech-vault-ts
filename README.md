...
Built with the latest **Next.js 15 App Router** and **React 19**, this repository represents the **TypeScript implementation** of the project. It features **End-to-End Type Safety**, **Role-Based Access Control (RBAC)**, **runtime payload validation with Zod**, and relational database modeling using **Drizzle ORM** with **Neon Serverless Postgres**.
...
### 1. 🛡️ End-to-End Type Safety & Runtime Validation
* **TypeScript & Zod Integration:** Bridges client-side form submissions and server-side mutations safely. All incoming `FormData` payloads are strictly parsed and validated at runtime using customized **Zod** schemas (`SignupSchema`, `LibraryItemScheme`) before interacting with the database.
* **Strict Type Definitions:** Complete TypeScript coverage across API responses, Zustand UI stores, database tables, and React component props (`myTypes.ts`).

### 2. 🔐 Role-Based Access Control (RBAC) & Secure Auth
* **Cookie-Based Authentication:** Implements custom, lightweight authentication using encrypted HTTP-only, SameSite, secure browser cookies (`cookies().set('auth-token', ...)`).
* **Granular Authorization Logic:** Distinguishes between `admin` and `user` roles:
  * **Users:** Can create libraries (defaulting to `pending` status) and can only edit or delete non-protected libraries that they originally created.
  * **Admins:** Possess full override capabilities, including updating any library, toggling protection locks (`isProtected`), and managing visibility badges (`public`, `private`, `pending`).

### 3. 🗄️ Relational Database Modeling via Drizzle ORM
...
### 4. ⚡ Bleeding-Edge Next.js 15 & React 19 Patterns
...
### 5. 🎛️ Hybrid State Management (URL + Client Store)
...
│   ├── app/
│   │   ├── auth/
│   │   │   └── action.ts         # Authentication Server Actions (signInAction, signUpAction, signOutAction)
│   │   ├── mylib/
...
│   │   ├── SignInModal.tsx       # User Login modal using `useActionState` and Zod feedback
│   │   ├── SignUpModal.tsx       # User Registration modal with automated cookie session creation
│   │   ├── myTypes.ts            # Centralized TypeScript interfaces, types, and Zod validation schemas
│   │   ├── neon.ts               # Drizzle ORM database connection, relational queries, and Auth helpers
